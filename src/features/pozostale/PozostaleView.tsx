...
export function PozostaleView() {
  ...
  const groups: GroupSummary[] = useMemo(() => {
    const groupMap = new Map<string, PozostaleItem[]>();
    for (const g of customGroups) {
      if (!groupMap.has(g)) groupMap.set(g, []);
    }
    for (const item of pozostale) {
  ...
  }, [pozostale, customGroups]);

  const allGroupNames = useMemo(() => {
    const names = new Set<string>();
    for (const item of pozostale) { if (item.grupa) names.add(item.grupa); }
    ...
    <Box>
      <Box sx={{ mb: 5 }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <Box>
            <Typography variant="h2">Inne</Typography>
            <Typography variant="body1" color="text.secondary">Dodatkowe koszty i wydatki</Typography>
          </Box>
      ...
      {pozostale.length === 0 && groups.length === 0 ? (
        <Card>
          <CardContent sx={{ py: 8, textAlign: 'center' }}>
            <MoreIcon sx={{ fontSize: 48, color: 'text.secondary', opacity: 0.3, mb: 1 }} />
                        ...
                        group.items.map((item) => (
                          <TableRow key={item.id} sx={{ opacity: item.included ? 1 : 0.4 }}>
                            <TableCell padding="checkbox">
                              <Checkbox checked={item.included} onChange={() => updatePozostaleItem(item.id, { included: !item.included })} size="small" />
                            </TableCell>
                            ...
                            <TableCell>
                              <Chip
                                ...
                                onClick={() => updatePozostaleItem(item.id, { status: item.status === 'Opłacone' ? 'Do zapłaty' : 'Opłacone' })}
                              />
                            </TableCell>
                            <TableCell>
                              <TextField size="small" value={item.uwagi} onChange={(e) => updatePozostaleItem(item.id, { uwagi: e.target.value })} placeholder="..." variant="standard" sx={{ '& .MuiInput-root': { fontSize: '0.8rem' } }} />
                            </TableCell>
                            <TableCell>
                            ...
                            <TableCell>
                              <Box sx={{ display: 'flex', gap: 0.5 }}>
                                ...
                                <Tooltip title="Usuń"><IconButton size="small" onClick={() => deletePozostaleItem(item.id)} sx={{ opacity: 0.5, '&:hover': { opacity: 1, color: theme.palette.error.main } }}><DeleteIcon fontSize="small" /></IconButton></Tooltip>
                              </Box>
                            </TableCell>
                          </TableRow>
                        ))
      ...
      <Dialog open={dialogOpen} onClose={() => { setDialogOpen(false); setEditingItem(null); }} maxWidth="sm" fullWidth>
        <DialogTitle sx={{ fontWeight: 600 }}>{editingItem ? 'Edytuj pozycję — Inne' : 'Dodaj pozycję — Inne'}</DialogTitle>
      ...
      <Dialog open={groupDialogOpen} onClose={() => setGroupDialogOpen(false)} maxWidth="xs" fullWidth>
        ...
        <DialogContent sx={{ pt: 1 }}>
          ...
          <TextField label="Nazwa grupy" value={newGroupName} onChange={(e) => setNewGroupName(e.target.value)} placeholder="np. Transport, Przeprowadzka..." fullWidth autoFocus onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); handleAddGroup(); } }} />
        </DialogContent>
        <DialogActions sx={{ px: 3, pb: 2.5 }}>
      ...
      <AlternativesModal
        ...
        onSave={(alternatywy) => {
          if (altItem) updatePozostaleItem(altItem.id, { alternatywy });
          setAltItem(null);
        }}
      ...
      <LinksModal
        ...
        onSave={(linki) => {
          if (linksItem) updatePozostaleItem(linksItem.id, { linki });
          setLinksItem(null);
        }}
