...
export function CostCategoryView({ config, items, updateItem, addItem, deleteItem }: CostCategoryViewProps) {
  ...
  const handleSubmit = () => {
    if (editingItem) {
      ...
      const newItem: Record<string, unknown> = { included: true, [costField]: Number(formData[costField]) || 0, status: 'Do zapłaty', uwagi: formData.uwagi || '', linki: [], alternatywy: [], wybranaAltId: null };
                                  ...
                                  <Box sx={{ pl: 2, pr: 2, pb: 1.5, pt: 0.5, display: 'flex', flexDirection: 'column', gap: 0.75 }}>
                                    ...
                                    {(item.alternatywy || []).map((alt) => (
                                      <Box key={alt.id} sx={{ display: 'grid', gridTemplateColumns: '24px 50px 1fr 120px 80px 100px', alignItems: 'center', gap: 2, px: 1.5, py: 0.75, borderRadius: 1.5, backgroundColor: alpha(theme.palette.info.main, 0.02), border: `1px solid ${alpha(theme.palette.info.main, 0.08)}`, opacity: alt.included ? 1 : 0.4 }}>
                                        ...
                                        <Box sx={{ display: 'flex', gap: 0.25, justifyContent: 'flex-end' }}>
                                          <Tooltip title="Ustaw jako MAIN"><IconButton size="small" onClick={() => { const oldMain: AlternativeItem = { id: generateId(), included: true, nazwa: 'Poprzednia opcja', cena: getCost(item), linki: item.linki || [], uwagi: '' }; const newAlts = [oldMain, ...(item.alternatywy || []).filter((a) => a.id !== alt.id)]; updateItem(item.id, { [costField]: alt.cena, linki: alt.linki || [], alternatywy: newAlts } as Partial<CostItem>); }} sx={{ opacity: 0.4, '&:hover': { opacity: 1, color: theme.palette.success.main }, width: 20, height: 20 }}><AltIcon sx={{ fontSize: 12 }} /></IconButton></Tooltip>
        ...
        <DialogContent dividers sx={{ maxHeight: '70vh' }}>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
            ...
            {(payChoiceItem?.alternatywy || []).filter(a => a.included).map((alt) => (
              <Box key={alt.id} onClick={() => { if (payChoiceItem) { const oldMain: AlternativeItem = { id: payChoiceItem.id + '_old', included: true, nazwa: 'Poprzednia cena', cena: getCost(payChoiceItem), linki: payChoiceItem.linki || [], uwagi: '' }; const newAlts = [oldMain, ...(payChoiceItem.alternatywy || []).filter(a => a.id !== alt.id)]; updateItem(payChoiceItem.id, { status: 'Opłacone', [costField]: alt.cena, linki: alt.linki || [], alternatywy: newAlts, wybranaAltId: null } as Partial<CostItem>); setPayChoiceItem(null); } }} sx={{ p: 1.5, borderRadius: 2, cursor: 'pointer', border: `1px solid ${theme.palette.divider}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center', '&:hover': { borderColor: theme.palette.info.main, backgroundColor: alpha(theme.palette.info.main, 0.04) } }}>
